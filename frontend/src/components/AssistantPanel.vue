...
import type { AssistantAction, AssistantInboxItem, AssistantMessage, AssistantSession } from "@/stores/workspace";
...
const visibleInboxItems = computed(() =>
  props.inboxItems.filter((item) => !archivedIds.value.has(item.id) && !item.archived),
);
  ...
  <div class="flex h-full flex-col bg-white">
    <div class="border-b border-border px-4 py-3">
      <div class="flex items-center justify-between gap-3">
        ...
        >
          <span>{{ t("assistantPanel.inbox") }}</span>
          <span class="rounded-full bg-warn px-1.5 py-0.5 text-[10px] font-bold leading-none text-white">{{ inboxUnreadTotal }}</span>
        </button>
      </div>
      ...
      >
        <div class="flex items-start justify-between gap-2">
          ...
          <button
            ...
            :title="t('assistantPanel.archiveItem')"
        ...
        <div class="mt-2 flex flex-wrap gap-1.5">
          ...
          >
            {{ t("common.switchTo") }} {{ t("common.tasks") }}
          </button>
        </div>
      </div>
    </div>
