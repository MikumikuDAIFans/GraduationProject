这份文档是为你量身定制的**《基于多Agent协同的智能个人事务助理系统架构设计与实施规范》**。

请将此文档保存为 Markdown (`PROJECT_SPEC.md`) 或 PDF。在后续开发中，你可以直接将这份文档发给 AI 编程助手（如 Cursor, Windsurf, 或继续询问我），作为项目的**核心指令集**。
...
- **外部 API**：
  - LLM：OpenAI 格式接口 (DeepSeek/ChatGPT/Claude)
  - Map：高德地图 API 腾讯地图API (Web 服务端)
...
3. 编写后端 `Dockerfile`：
   ...
   - 安装依赖：`django`, `djangorestframework`, `mysqlclient`, `channels`, `daphne`, `langchain`, `openai`, `celery`, `redis`.
4. **验证**：运行 `docker-compose up -d`，确保所有服务状态为 Up。
...
2. **Agent 编排**：
   - 使用 `langchain.agents.initialize_agent`。
...
1. **Docker 生产配置**：
   - 配置 Nginx 容器作为反向代理，处理静态文件和 API 转发。
2. **Capacitor 集成**：
